...
export const saveUserToJsonServer = newUser => {
  return (
    ...
    fetch(`${url}`, {
      ...
      .then(response => {
        return fetch(
          `https://usability-tree-testing.firebaseio.com/users/${response.name}.json`
        );
      })
      .then(r => r.json())
...
export const getUser = userId => {
  return fetch(
    `https://usability-tree-testing.firebaseio.com/users/${userId}.json?&print=pretty`
...
export const login = (email, password) => {
  return (
    ...
    firebase
      ...
      .then(user => {
        return fetch(
          `https://usability-tree-testing.firebaseio.com/users.json?orderBy="id"&equalTo="${user.user.uid}"&print=pretty`
        );
      })
      .then(results => results.json())
...
export const register = newUser => {
  ...
  return (
    registerWithFB(newUser.email, newUser.password)
